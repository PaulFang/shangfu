<!DOCTYPE HTML>
<html>
<head>
<meta http-equiv="Content-Type" content="text/html; charset=utf-8">
<link href="css/base.css" rel="stylesheet" type="text/css">

<link href="css/product.css" rel="stylesheet" type="text/css">
<script type="text/javascript" src="js/jquery-1.7.2.min.js"></script>
<script type="text/javascript" src="js/easing.js"></script>
<link type="text/css" rel="stylesheet" href="css/cloud-zoom.css">
<script type="text/javascript" src="js/cloud-zoom.1.0.2.js"></script>
<title>尚孚·一品生活-茶油-产品展示-尚孚茶油</title>
</head>
<body>
<script type="text/javascript">
    var loads=document.getElementById("ioad").getElementsByTagName("span");
    var loadIndex=0;
    var loadLen=loads.length;
    setInterval(function(){
        loadIndex++;
        loadIndex=loadIndex>9?0:loadIndex;
        for(var i=0;i<loadLen;i++){
            var dom=loads[i];
            if(dom){
                dom.style.backgroundPosition="center -"+(40*loadIndex)+"px";
            }
        }
    },40);
</script>
<div class="header">
    <div class="main">
        <div class="logo"><a href="index"></a></div>
		<div style="width:650px;float:left;">
			<ul class="nav" style="margin-top: 14px;">
				<li><a href="about" >关于尚孚</a></li>
				<li><a href="product" class="current">产品展示</a></li>
				<li><a href="honor" >荣誉资质</a></li>
				<li><a href="news" >新闻资讯</a></li>
				<li><a href="contact" >联系我们</a></li>
			</ul>
		</div>
        
        <div class="slogan" > <img src="images/slogan.png" style="width:280px;height:59px;"> </div>
    </div>
</div>
<div class="wrapper">
    <div class="banner" style="background-image: url(images/neck_banner/product_banner.jpg)">
    </div>
    <div class="main">
        <div class="location">
            <a href="index">首页</a>
            <em>&gt;</em>
            <span><a href="product">产品展示</a></span>
			<em>&gt;</em>
			<span><a href="#">茶油</a></span>
			<em>&gt;</em>
			<span>尚孚·一品生活</span>
        </div>
        <div class="product-detail">
            
            <div class="photos">
                <div class="detail">
				   <a href='product_img/chuyun/main.jpg' class='cloud-zoom' id='zoom1' rel="adjustX:10, adjustY:-4">
				    <img src="product_img/chuyun/main.jpg">
				   </a>
				</div>
				<div class="photoList">
                    <a href="javascript:void(0)" class="butLeft">&lt;</a>
                    <a href="javascript:void(0)" class="butRight">&gt;</a>
                    <div class="contents">
                        <ul>
						    <li class="current">
							    <a href='product_img/chuyun/1.jpg' class='cloud-zoom-gallery' rel="useZoom: 'zoom1', smallImage: 'product_img/chuyun/1.jpg' ">
								   <img src="product_img/chuyun/1.jpg"/>
								</a>
                            </li>
							
							<li class="current">
							    <a href='product_img/chuyun/2.jpg' class='cloud-zoom-gallery' rel="useZoom: 'zoom1', smallImage: 'product_img/chuyun/2.jpg' ">
								   <img src="product_img/chuyun/2.jpg"/>
								</a>
                            </li>
							<li class="current">
							    <a href='product_img/chuyun/3.jpg' class='cloud-zoom-gallery' rel="useZoom: 'zoom1', smallImage: 'product_img/chuyun/3.jpg' ">
								   <img src="product_img/chuyun/3.jpg"/>
								</a>
                            </li>
							<li class="current">
							    <a href='product_img/chuyun/4.jpg' class='cloud-zoom-gallery' rel="useZoom: 'zoom1', smallImage: 'product_img/chuyun/4.jpg' ">
								   <img src="product_img/chuyun/4.jpg"/>
								</a>
                            </li>
							<li class="current">
							    <a href='product_img/chuyun/main.jpg' class='cloud-zoom-gallery' rel="useZoom: 'zoom1', smallImage: 'product_img/chuyun/main.jpg' ">
								   <img src="product_img/chuyun/main.jpg"/>
								</a>
                            </li>
							
							                       </ul>
                    </div>
                </div>
               
            </div>
            <div class="info">
                <h2>尚孚·厨韵</h2>
                <table>
                    <colgroup>
                        <col width="30">
                        <col width="130">
                        <col width="140">
                        <col>
                    </colgroup>
                    
                    <tbody>
					<tr>
                        <td><i></i></td>
                        <th>系列：</th>
                        <th>尚孚·厨韵</th>
                       
                    </tr>  
					<tr>
                        <td><i></i></td>
                        <th>包装：</th>
                        <th>普通包装</th>
                        
                    </tr><tr>
                        <td><i></i></td>
                        <th>规格：</th>
                        <th>500ml</th>
                       
                    </tr> 
					<!-- <tr>
                        <td><i></i></td>
                        <th>价格：</th>
                        <th>￥618.00</th>
                        
                    </tr> -->					</tbody>
                </table>
                <p> 原生态臻品，点缀家居新厨意，尚孚野生山茶油，带您领略舌尖的艺术，尊享一品生活。</p>
                <p>原味最赖蒸煮，清炒自然留芳，最是色香味美处，尚孚厨韵满厅堂。</p>
                <p>有家自有厨房，有厨当有尚孚。</p>
                <p>尚孚.厨韵系列，专为居家生活倾力奉送的经济适用型茶油，留下一份温情与趣味，与您共享天伦之乐。</p>
                <div class="cartBox">
                    <a href="https://detail.1688.com/offer/520441772373" target="_blank" class="cart" style=" margin-left:30px;"><i></i>立即订购</a>
                    
                    
                </div>
                
            </div>
        </div>
    </div>
</div>
<div class="footer">
    <div class="main">
        <div class="tel">0713-8533399 17771339998</div>
        <div class="right">
            <ul class="menu-foot">
                <li><a href="about" class="current">关于尚孚</a></li>
                <li><a href="product" >产品展示</a></li>
                <li><a href="honor" >荣誉资质</a></li>
                <li><a href="news" >新闻资讯</a></li>
                <li><a href="contact" >联系我们</a></li>
            </ul>
            <div class="copyright">
                <strong>Copyright © 2017 www.xianpin365.com 粤ICP备15001240号 All Rights Reserved</strong>
                <!--
                <div class="share">
                    <div class="bdsharebuttonbox"><a href="#" class="bds_more" data-cmd="more"></a><a href="#" class="bds_tsina" data-cmd="tsina" title="分享到新浪微博"></a><a href="#" class="bds_tqq" data-cmd="tqq" title="分享到腾讯微博"></a><a href="#" class="bds_weixin" data-cmd="weixin" title="分享到微信"></a><a href="#" class="bds_qzone" data-cmd="qzone" title="分享到QQ空间"></a></div>
                </div>
                -->
            </div>
        </div>
    </div>
</div>
<input type="hidden" id="la" value="$_COOKIE['think_language']"/>
<script type="text/javascript" src="js/function.js"></script>
<script type="text/javascript" src="js/public.js"></script>
<script type="text/javascript">
    window._bd_share_config={"common":{"bdSnsKey":{},"bdText":"","bdMini":"2","bdMiniList":false,"bdPic":"","bdStyle":"1","bdSize":"16"},"share":{}};with(document)0[(getElementsByTagName('head')[0]||body).appendChild(createElement('script')).src='http://bdimg.share.baidu.com/static/api/js/share.js?v=89860593.js?cdnversion='+~(-new Date()/36e5)];
	
	
	/*$(function(){
	    $(document).on("click", "a", function () {
            var _l=$("#la").val();
            var _url=$(this).attr("href");
            if(_l=="en-us"){
			   $(this).attr("href",_url+'?l=en-us');
			}			
			
		});
	})*/
</script>

<script type="text/javascript">
    $(function(){
        $(".photoList").custoersWheel({time: -1});
        $(".contents a").click(function(){
            var li=$(this).parents("li")
            li.addClass("current").siblings("li").removeClass("current");
        });
        $(".collect a").click(function(){
            var _this=$(this);
            if(_this.hasClass("on")){
               return;
            }
		    var _id=_this.attr("data-id");
			var url="/Home/Index/zan";
			$.post(url,{id:_id},function(data){
			     $(".collect")(data);
			});
        });
        $(document).on("click",".codeBut",function(){
            var _this=$(this);
            if(_this.hasClass("on")){
                _this.removeClass("on");
                _this.animate({"top":0},300);
                $(".codeBox").animate({"top":-564},300);
            }else{
                _this.addClass("on");
                _this.animate({"top":-25},300);
                $(".codeBox").animate({"top":0},{
                    duration: 600,
                    easing: "easeOutCirc"
                });
            }
        });
		
    });
</script>
</body>
</html>